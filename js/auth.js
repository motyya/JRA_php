class AuthManager {
    ...
    async loadExistingFavorites() {
        ...
        try {
            // Лошади
            const horseResponse = await fetch(`/api/favorites.php?type=horses&user_id=${this.currentUser.id}`);
            if (horseResponse.ok) {
            ...
            // Забеги
            const raceResponse = await fetch(`/api/favorites.php?type=races&user_id=${this.currentUser.id}`);
            if (raceResponse.ok) {
            ...
            // Ипподромы
            const racecourseResponse = await fetch(`/api/favorites.php?type=racecourses&user_id=${this.currentUser.id}`);
    ...
    async toggleRacecourseFavorite(racecourseId, racecourseName, button) {
        ...
        try {
            ...
            const response = await fetch('/api/favorites.php', {
                ...
                body: JSON.stringify({
                    user_id: this.currentUser.id,
                    racecourse_id: parseInt(racecourseId),
                    type: 'racecourses'
                })
            });
    ...
    showNotification(message, type) {
        if (!window.CONFIG.DEBUG && type === 'success') return;
