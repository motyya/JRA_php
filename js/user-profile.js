class UserProfile {
    ...
    async loadUserProfile() {
        try {
            const response = await fetch(`/api/jockeys.php?id=${this.currentUser.id}`);
    ...
    async loadFavoriteHorses() {
        try {
            const response = await fetch(`/api/favorites.php?type=horses&user_id=${this.currentUser.id}`);
    ...
    async loadFavoriteRaces() {
        try {
            const response = await fetch(`/api/favorites.php?type=races&user_id=${this.currentUser.id}`);
    ...
    async loadFavoriteRacecourses() {
        try {
            const response = await fetch(`/api/favorites.php?type=racecourses&user_id=${this.currentUser.id}`);
    ...
    async loadRaceEntries() {
        try {
            const response = await fetch(`/api/race-entries.php?user_id=${this.currentUser.id}`);
                ...
                if (container) {
                    container.innerHTML = `
                        <div class="empty-state">
                            ...
                            <a href="?page=race-entry" class="btn-primary" style="margin-top: 1rem;">Submit Your First Entry</a>
                        </div>
                    `;
                }
                return;
