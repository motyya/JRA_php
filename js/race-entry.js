document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadFormData() {
        try {
            // Загружаем лошадей
            const horsesResponse = await fetch('/api/available.php?type=horses');
            ...
            // Загружаем забеги
            const racesResponse = await fetch('/api/available.php?type=races');
            if (racesResponse.ok) {
    ...
    async function handleSubmit(event) {
        ...
        // Автозаполнение из текущего пользователя
        ...
        try {
            const response = await fetch('/api/race-entries.php', {
                method: 'POST',
                headers: {
